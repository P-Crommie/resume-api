...
@patch('boto3.Session')
@patch('boto3.resource')
def test_lambda_handler_success(mock_boto3_resource, mock_boto3_session, mock_environment):
    # Mock DynamoDB Table and its get_item method
    mock_table = MagicMock()
    mock_table.get_item.return_value = {
        'Item': {'id': '1', 'name': 'Test Resume'}
...
@patch('boto3.Session')
@patch('boto3.resource')
def test_lambda_handler_resume_not_found(mock_boto3_resource, mock_boto3_session, mock_environment):
    # Mock DynamoDB Table and its get_item method to return no item
    mock_table = MagicMock()
    mock_table.get_item.return_value = {}
    mock_boto3_resource.return_value.Table.return_value = mock_table
...
@patch('boto3.Session')
@patch('boto3.resource')
def test_lambda_handler_internal_server_error(mock_boto3_resource, mock_boto3_session, mock_environment):
    # Mock DynamoDB Table to raise an exception
    mock_table = MagicMock()
    mock_table.get_item.side_effect = Exception("Test exception")
    mock_boto3_resource.return_value.Table.return_value = mock_table
